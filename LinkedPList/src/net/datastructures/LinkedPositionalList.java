...
public class LinkedPositionalList<E> implements PositionalList<E> {
	...
	/* this part added by Nick Jones */
	public void swap(Position<E> p, Position<E> q) {
		Node<E> nodeP = validate(p);
		Node<E> nodeQ = validate(q);

		/* First Deal With Adjacent nodes */
		if (nodeP == nodeQ.getPrev() || nodeQ == nodeP.getPrev()) {
			Node<E> first;
			Node<E> second;
			if (nodeP == nodeQ.getPrev()) {
				first = nodeP;
				second = nodeQ;
			} else {
				first = nodeQ;
			...
			/* Next Handle Non Adjacent Nodes */
	...
	public Node<E> reverseList2(Node<E> head) {
	    ...
	    Node<E> newHead = reverseList2(nextNode);
