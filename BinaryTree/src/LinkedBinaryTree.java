...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	public Position<E> preorderNext(Position<E> p) {
		...
		if (node != null) {
			...
			if (node.right != null)
				return node.right;
			return preorderMod(node.parent);
		}
		return null;
	}

	private Node<E> preorderMod(Node<E> temp) {
		if (temp != null && temp != root) {
			if (temp.parent.left == temp && temp.parent.right != null)
				return temp.parent.right;

			return preorderMod(temp.parent);
		}
		return null;
	}
	...
	public Position<E> postorderNext(Position<E> p) {
		Node<E> parent = validate(p);
		if (parent.right == null || parent.right == p)
			return parent;
		Node<E> curr = parent.right;
